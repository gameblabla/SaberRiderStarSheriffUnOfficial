...
void input_event(Input *in, const SDL_Event *ev)
{
    ...
    if (ev->type == SDL_EVENT_KEY_DOWN || ev->type == SDL_EVENT_KEY_UP) {
        ...
        switch (ev->key.scancode) {    /* demo keyboard layout: arrows move, W/A jump, S/D shoot, Q/E aim, Enter pause */
...
void input_update(Input *in)
{
    ...
    if (pad) {
        ...
        down[BTN_JUMP]  |= SDL_GetGamepadButton(pad, SDL_GAMEPAD_BUTTON_SOUTH) || SDL_GetGamepadButton(pad, SDL_GAMEPAD_BUTTON_NORTH);
