...
/* ---- tuning ---- */
#define WAIT_T     R(1.8f)     /* the gunship's wreck burns out, the music fades */
#define APPEAR_T   R(1.8f)     /* she forms out of the motes */
#define READY_T    R(0.9f)     /* after the scene: a beat before she moves */
#define DEATH_T    R(1.1f)     /* her death animation, then she dissolves */
#define DISSOLVE_T R(1.6f)
#define GAP_MIN    R(100.0f)   /* closer than this she backs off (a leap over the hero when cornered): she duels at range */
#define SHOT_SPEED R(250.0f)   /* the hero's shots fly at 500 */
#define SHOT_R     R(3.0f)     /* shot radius in this duel, both ways (5 elsewhere): a crouch ducks a level shot, as it
...
#ifdef REAL_FIXED
static real frand(void) { return (real)((uint32_t)rand() >> 15); }   /* rand() is 31 bits (libc_sat.c: glibc's): 16 of them */
#else
...
static void motes_update(DarkApril *d, real dt)
{
    for (int i = 0; i < DARK_MOTES; i++) {
        DarkMote *m = &d->motes[i];
        if (m->life <= 0) continue;
        m->life -= dt; m->x += r_mul_dt(m->vx, dt); m->y += r_mul_dt(m->vy, dt);
    }
}
...
void dark_begin(DarkApril *d, real arena_x, int sw, int difficulty)
{
    ...
    player_spawn(&d->p, CRHC_APRIL, arena_x + r_int(sw) - R(80.0f), R(120.0f));
    hero_quiet(false);
    d->p.quiet = true; d->p.locked = true;
    for (int b = 0; b < BTN_COUNT; b++) d->in.state[b] = 1;
    if (plat_getenv("SABER_TRACE")) fprintf(stderr, "dark april: arena %s, hp %d\n", RS(arena_x, 0), d->hp);
}
...
static int threat(const DarkApril *d, const Bullets *pb)
{
    ...
    real top = b->y + h->oy - h->hh, bot = b->y + h->oy + h->hh;
    for (int i = 0; i < pb->n; i++) {
        const Bullet *bl = &pb->b[i];
        int dir = bl->dir & 7;
        real vx = (dir == 0 || dir == 1 || dir == 7) ? R(-1.0f) : (dir == 3 || dir == 4 || dir == 5) ? R(1.0f) : R(0.0f);
        if (vx == 0) continue;
        real ahead = r_mul(b->x - bl->x, vx);
        if (ahead < R(-8.0f) || ahead > R(130.0f)) continue;
        real t = r_div(ahead, bl->speed > R(1) ? bl->speed : R(1));
        real y = bl->y + r_mul(r_mul(r_mul((dir == 1 || dir == 3) ? R(-1.0f) : (dir == 5 || dir == 7) ? R(1.0f) : R(0.0f), bl->speed), R(0.7f)), t);
...
static void pick_mode(DarkApril *d, const Character *P, real adx, bool phase2)
{
    real r = frand();
    bool grounded = on_ground(P);
    if (grounded && adx > R(130.0f) && adx < R(220.0f) && r < (phase2 ? R(0.25f) : R(0.15f))) d->mode = M_SLIDE;
    else if (r < (phase2 ? R(0.55f) : R(0.6f))) d->mode = M_MIRROR;
    else d->mode = M_ATTACK;
    d->mode_t = d->mode == M_MIRROR ? R(1.6f) + r_mul(frand(), R(1.8f)) : d->mode == M_ATTACK ? R(1.2f) + r_mul(frand(), R(1.3f)) : R(1.0f);
    d->pref = R(130.0f) + r_mul(frand(), R(70.0f));
    d->crouch_shoot = frand() < R(0.35f);
}

static void think(DarkApril *d, const Player *pl, const Bullets *pb, real dt)
{
    Character *c = &d->p.ch; const Character *P = &pl->ch;
    bool *w = d->want;
    memset(w, 0, sizeof d->want);
    real dx = P->body.x - c->body.x, adx = r_abs(dx);
    bool right = dx > 0, phase2 = d->hp * 2 <= d->hp_max;
    int toward = right ? BTN_RIGHT : BTN_LEFT, away = right ? BTN_LEFT : BTN_RIGHT;
    real left_room = c->body.x - d->arena_x, right_room = d->arena_x + r_int(d->sw) - c->body.x;
    bool player_down = P->state == CS_DEAD;

    d->mode_t -= dt; d->dodge_cd -= dt; d->counter_t -= dt;
    real back_room = right ? left_room : right_room;
    if (adx < GAP_MIN && on_ground(c) && c->state != CS_SLIDE && d->mode != M_LEAP && d->mode != M_RETREAT && !player_down) {
        d->mode = M_RETREAT; d->mode_t = R(2.0f); d->pref = R(140.0f) + r_mul(frand(), R(50.0f)); d->counter_t = 0; d->dodge_t = 0;
    }
    ...
    if (level_fire && d->counter_t <= R(-0.4f) && d->mode != M_SLIDE && d->mode != M_LEAP && d->mode != M_RETREAT) {
        real chance = (d->difficulty == 0 ? R(0.35f) : d->difficulty == 1 ? R(0.55f) : R(0.7f)) + (phase2 ? R(0.15f) : R(0.0f));
        d->counter_t = frand() < chance ? R(0.7f) + r_mul(frand(), R(0.5f)) : R(0.0f);
    }
    ...
    if (d->player_idle > R(1.1f) && d->mode == M_MIRROR) { d->mode = M_ATTACK; d->mode_t = R(1.4f); d->pref = R(100.0f) + r_mul(frand(), R(60.0f)); }

    /* dodging comes first: one roll per shot she sees coming (the cooldown), a better eye in the second half */
    if (d->dodge_t > 0) {
        d->dodge_t -= dt;
        if (d->dodge == DG_CROUCH) { w[BTN_DOWN] = true; w[BTN_SHOOT] = !player_down && frand() < R(0.5f) && c->facing == (right ? 1 : 0); return; }
        if (d->dodge == DG_JUMP) { w[BTN_JUMP] = d->dodge_t > R(0.25f); if (back_room > R(60.0f)) w[away] = true; return; }   /* hop back, not at the hero */
    }
    if (d->dodge_cd <= 0 && on_ground(c) && c->state != CS_SLIDE) {
        int th = threat(d, pb);
        if (th) {
            d->dodge_cd = R(0.45f);
            real chance = (d->difficulty == 0 ? R(0.35f) : d->difficulty == 1 ? R(0.55f) : R(0.7f)) + (phase2 ? R(0.12f) : R(0.0f));
            if (frand() < chance) { d->dodge = th; d->dodge_t = th == DG_CROUCH ? R(0.45f) : R(0.3f); think(d, pl, pb, 0); return; }
        }
    }
    ...
    case M_MIRROR: {   /* the player's own buttons a moment ago, left and right swapped */
        ...
        if (adx < GAP_MIN + R(40.0f) && !w[BTN_AIM]) w[toward] = false;   /* mirrored, but she won't walk into the hero */
        break; }
    case M_ATTACK:
        if (player_down) { w[BTN_AIM] = true; w[toward] = true; break; }
        if (adx > d->pref + R(24.0f)) {                        /* close in, firing on the run */
            w[toward] = true; w[BTN_SHOOT] = frand() < R(0.6f);
            ...
            real dy = (P->body.y - R(10.0f)) - c->body.y;
            w[BTN_AIM] = true; w[BTN_SHOOT] = true;
            if (dy < r_mul(-adx, R(0.45f))) { w[BTN_UP] = true; if (dy > r_mul(-adx, R(2.4f))) w[toward] = true; }
            else w[toward] = true;
        }
        break;
    case M_SLIDE:      /* down + toward, then jump: April's slide, right into the hero */
        if (c->state == CS_SLIDE) { w[toward] = false; break; }
        if (d->mode_t < R(0.9f) && on_ground(c)) { d->mode = M_RETREAT; d->mode_t = R(2.0f); d->pref = R(150.0f); break; }   /* through the hero: then away */
        w[toward] = true; w[BTN_DOWN] = true; w[BTN_JUMP] = true;
        break;
    case M_LEAP:       /* cornered: a somersault over the hero, then away on the other side */
        if (on_ground(c) && d->mode_t < R(0.8f)) { d->mode = M_RETREAT; d->mode_t = R(2.0f); d->pref = R(150.0f); break; }
        w[toward] = true; w[BTN_JUMP] = on_ground(c);
        break;
    case M_RETREAT:    /* back off to her range, then turn and duel */
        if (adx >= d->pref || d->mode_t <= 0 || (back_room < R(40.0f) && adx >= GAP_MIN)) {
            d->mode = M_ATTACK; d->mode_t = R(1.2f) + frand(); w[BTN_AIM] = true; w[toward] = true; break;
        }
        if (back_room < R(40.0f)) { d->mode = M_LEAP; d->mode_t = R(1.0f); w[toward] = true; w[BTN_JUMP] = on_ground(c); break; }
        w[away] = true;
        break;
    }
    /* the arena's walls: backed into one she jumps out toward the middle */
    bool into_left = w[BTN_LEFT] && left_room < R(28.0f), into_right = w[BTN_RIGHT] && right_room < R(28.0f);
...
static void fire(DarkApril *d, Bullets *eb, Effects *fx, int layer)
{
    ...
    p->fire_cooldown = (phase2 ? R(0.3f) : R(0.42f)) + (d->difficulty == 0 ? R(0.12f) : d->difficulty == 2 ? R(-0.06f) : R(0.0f));
    sfx_play(1, 0);
}
...
static bool overlap(const Character *a, const Character *b)
{
    ...
    return r_abs(a->body.x + ha->ox - b->body.x - hb->ox) <= ha->hw + hb->hw && r_abs(a->body.y + ha->oy - b->body.y - hb->oy) <= ha->hh + hb->hh;
}

static void take_hit(DarkApril *d, int dmg)
{
    d->hp -= dmg; d->flash = R(0.1f);
    if (d->voice_t <= 0) { static const char *const H[3] = { "dark_hurt1", "dark_hurt2", "dark_hurt3" }; voice(H[rand() % 3]); d->voice_t = R(0.9f); }
    if (d->mode == M_MIRROR && frand() < R(0.3f)) { d->mode = M_ATTACK; d->mode_t = R(0.9f); d->pref = R(120.0f); }   /* stung: she answers */
}

/* ours: a hero's power attack (power.c) catches her mid-fight: frac of her hit points */
void dark_power_hit(DarkApril *d, real frac)
{
    if (d->state != DA_FIGHT) return;
    take_hit(d, r_ceil(d->hp_max * frac));
    d->flash = R(0.3f);
}

static void body_step(DarkApril *d, const Level *L, const PhysicsWorld *W, real dt)
{
    Character *c = &d->p.ch;
    PhysicsWorld aw = *W; aw.world_min_x = d->arena_x; aw.world_max_x = d->arena_x + r_int(d->sw);
    player_resolve(c, dt);
    physics_step(&aw, L, &c->body, dt);
    real lo = d->arena_x + R(12.0f), hi = d->arena_x + r_int(d->sw) - R(12.0f);
    if (c->body.x < lo) c->body.x = lo;
    if (c->body.x > hi) c->body.x = hi;
    character_animate(c, dt);
}

static void face(Character *c, real x) { c->facing = x > c->body.x ? 1 : 0; c->aim = c->facing ? AIM_R : AIM_L; }

void dark_animate(DarkApril *d, real dt)
{
                 ...
                 Bullets *eb, Effects *fx, int layer, real dt, bool live)
{
    ...
    switch (d->state) {
    case DA_WAIT:
        music_set_volume(R(1.0f) - r_div(d->t, WAIT_T));
        if (!d->settled) {   /* she forms on the floor on the far side from the hero */
            real x = pl->ch.body.x < d->arena_x + r_int(d->sw) / 2 ? d->arena_x + r_int(d->sw) - R(84.0f) : d->arena_x + R(84.0f);   /* sw * 0.5f */
            c->body.x = x; c->body.y = R(120.0f);
            PhysicsWorld aw = *W; aw.world_min_x = d->arena_x; aw.world_max_x = d->arena_x + r_int(d->sw);
            for (int i = 0; i < 240 && !(c->body.coll & COLL_DOWN); i++) { character_sync_ground(c); player_resolve(c, R_DT); physics_step(&aw, L, &c->body, R_DT); }
            character_sync_ground(c); face(c, pl->ch.body.x); player_resolve(c, 0); character_animate(c, 0);
            d->settled = true;
        }
        if (d->t >= WAIT_T && live && pl->ch.state != CS_DEAD) { music_stop(); music_set_volume(R(1)); d->state = DA_CALL; d->scene_call = true; }
        break;
    case DA_CALL:        /* the game shows the radio call; once it is closed she forms */
        if (live && !d->scene_call) d->state = DA_APPEAR;
        break;
    case DA_APPEAR: {
        real k = r_div(d->t, APPEAR_T); d->alpha = k < R(1) ? r_mul(k, k) : R(1);
        const HurtBox *h = hurt(c);
        for (int i = 0; i < 2 && k < R(0.85f); i++) {   /* motes drawn in toward her from all around */
            real a = r_mul(frand(), R(6.2831853f)), r = R(40.0f) + r_mul(frand(), R(50.0f));
            real tx = c->body.x + h->ox + r_mul(frand() - R(0.5f), R(20.0f)), ty = c->body.y + h->oy + r_mul(frand() - R(0.5f), R(44.0f));
            mote(d, tx + r_mul(r_cos(a), r), ty + r_mul(r_sin(a), r), r_div(r_mul(-r_cos(a), r), R(0.6f)), r_div(r_mul(-r_sin(a), r), R(0.6f)), R(0.6f));
        }
    ...
    case DA_READY:
        face(c, pl->ch.body.x); character_sync_ground(c); player_resolve(c, dt); character_animate(c, dt);
        if (d->t >= READY_T) { d->state = DA_FIGHT; d->p.locked = false; music_play(8, true); d->mode = M_MIRROR; d->mode_t = R(1.2f); }
        break;
    case DA_FIGHT: {
        ...
        for (int i = 0; i < pb->n; i++) {
            Bullet *bl = &pb->b[i];
            if (r_abs(c->body.x + h->ox - bl->x) > SHOT_R + h->hw || r_abs(c->body.y + h->oy - bl->y) > SHOT_R + h->hh) continue;
            pb->b[i] = pb->b[--pb->n];
            take_hit(d, 1);
            break;
        }
        ...
        if (pl->ch.state != CS_DEAD && overlap(c, &pl->ch)) {
            if (pl->ch.state == CS_SLIDE && c->state != CS_SLIDE) { if (d->iframes <= 0) { take_hit(d, 1); d->iframes = R(0.8f); } }
        ...
        if ((d->ghost_t -= dt) <= 0 && (fast || d->hp * 2 <= d->hp_max)) {
            ...
            d->ghost_t = R(0.05f);
        } else if (!fast && d->hp * 2 > d->hp_max && d->ghost_t < R(-0.2f)) d->nghost = 0;
        if (d->hp <= 0) {
        ...
        break; }
    case DA_DYING: {
        music_set_volume(R(1.0f) - r_div(d->t, DEATH_T + DISSOLVE_T));
        character_sync_ground(c); c->state = CS_DEAD; body_step(d, L, W, dt);
        real k = r_div(d->t - DEATH_T, DISSOLVE_T);
        if (k > 0) {
            d->alpha = k < R(1) ? R(1) - k : 0;
            const HurtBox *h = hurt(c);
            for (int i = 0; i < 3 && k < 1; i++)   /* she comes apart in rising motes */
                mote(d, c->body.x + h->ox + r_mul((frand() - R(0.5f)) * 2, h->hw), c->body.y + h->oy + r_mul((frand() - R(0.5f)) * 2, h->hh),
                     r_mul(frand() - R(0.5f), R(30.0f)), R(-30.0f) - r_mul(frand(), R(50.0f)), R(0.5f) + r_mul(frand(), R(0.7f)));
        }
        if (k >= R(1.3f)) { d->state = DA_GONE; music_stop(); music_set_volume(R(1)); }
        break; }
    case DA_GONE:
        if (d->t > R(0.8f) && live && pl->ch.state != CS_DEAD) { d->state = DA_DONE; d->scene_outro = true; }
        break;
    }
    if (d->state != prev) {
        d->t = 0;
    ...
    if (plat_getenv("SABER_TRACE") && d->state == DA_FIGHT)
        fprintf(stderr, "dark: mode=%d st=%d x=%s y=%s hp=%d hero=%d/%d in=%d%d%d%d%d%d%d\n", d->mode, c->state, RS(c->body.x, 0), RS(c->body.y, 0), d->hp, pl->hp, pl->lives,
...
static void draw_as(const Character *c, real cx, real cy, uint8_t r, uint8_t g, uint8_t b, uint8_t a, RBlend m)
{
...
void dark_draw(const DarkApril *d, real cam_x, real cam_y)
{
    ...
    if (d->state != DA_DONE && d->state != DA_GONE && d->state != DA_WAIT && d->state != DA_CALL) {
        ...
        for (int i = 0; i < d->nghost; i++) {   /* oldest first, faintest */
            ...
            draw_as(&d->ghost[k], cam_x, cam_y, 110, 30, 170, (uint8_t)r_trunc(a * (40 + 30 * i)), R_BLEND_ADD);
        }
        /* a violet glow a pixel out all round, then the body as a dark shadow of April, a white flash on a hit */
        uint8_t ga = (uint8_t)r_trunc(r_mul(a, R(190) + 50 * r_sin(r_mul(d->t, R(6.0f)))));
        draw_as(c, cam_x - R(1), cam_y, 150, 40, 230, ga, R_BLEND_ADD);
        draw_as(c, cam_x + R(1), cam_y, 150, 40, 230, ga, R_BLEND_ADD);
        draw_as(c, cam_x, cam_y - R(1), 150, 40, 230, ga, R_BLEND_ADD);
        draw_as(c, cam_x, cam_y + R(1), 150, 40, 230, ga, R_BLEND_ADD);
        draw_as(c, cam_x, cam_y, 84, 46, 118, (uint8_t)r_trunc(a * 255), R_BLEND_BLEND);
        if (d->flash > 0) draw_as(c, cam_x, cam_y, 255, 255, 255, (uint8_t)r_trunc(a * 200), R_BLEND_ADD);
    }
    if (!ren) return;
    ...
    for (int i = 0; i < DARK_MOTES; i++) {
        ...
        if (m->life <= 0) continue;
        real k = r_div(m->life, m->max);
        r_set_draw_color(ren, (uint8_t)r_trunc(170 * k + R(40)), (uint8_t)r_trunc(60 * k), (uint8_t)r_trunc(255 * k), 255);
        RFRect q = { r_floorr(m->x - cam_x), r_floorr(m->y - cam_y), k > R(0.5f) ? R(2.0f) : R(1.0f), k > R(0.5f) ? R(2.0f) : R(1.0f) };
        r_fill_rect(ren, &q);
    }
...
void dark_draw_hud(const DarkApril *d, Ren *ren, int sw)
{
    ...
    real k = d->hp > 0 ? r_int(d->hp) / d->hp_max : 0;
    RFRect fg = { x, y, r_mul(bw, k), R(5) };
    r_fill_rect(ren, &fg);
}
