...
#define NIGHT_INTRO_CAM R(48.0f)
#define NIGHT_TAUNT_X R(6480.0f)
...
typedef struct {
    Stage3World world;
    int difficulty;
    real start_x, start_y, intro_stop_x;
    ...
    HyperState state;
    real st;                   /* time in the current state */
    real arena_x;              /* camera x of the arena (fixed once the fight starts) */
    int sw;
    real bx, by, speed;        /* ship centre (world), current speed */
    int dir;                    /* -1 flying/facing left (art as drawn), +1 right (mirrored) */
    int cycle;                  /* attack round: sides and passes alternate */
    int hp, hp_max;
    real hit_flash;
    bool shooting; real fire_t;   /* gun cycle: running this step, time to the next shot */
    real death_vy; int death_phase; bool death_front;
...
void night_power_hit(Night *n, Effects *fx, real frac, bool clear_shots);   /* a hero's power attack (power.c) */   /* far pass, mid pass, then the fight over [arena_x, arena_x + sw] */
...
void night_draw_layer(Night *n, Ren *ren, int layer, real cam_x, real cam_y);   /* boss + shots in their sprite layer */
