...
static void usage(const char *exe)
{
    fprintf(stderr, "usage: %s [data_dir] [--level N]\n"
                    ...
                    "  --level N   skip the front end and start at level N (1 the frontier town, 2 the All Galaxy Grand Prix, 3 Hyperjumper Pass)\n", exe);
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--level") || !strcmp(argv[i], "-l")) {
            if (i + 1 >= argc || (start_level = atoi(argv[++i])) < 1 || start_level > 3) { usage(argv[0]); return 2; }
        } else if (!strncmp(argv[i], "--level=", 8)) { start_level = atoi(argv[i] + 8); if (start_level < 1 || start_level > 3) { usage(argv[0]); return 2; } }
