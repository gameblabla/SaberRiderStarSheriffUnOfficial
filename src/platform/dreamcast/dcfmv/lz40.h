 ...
 *
 * Algorithm from https://github.com/VincentNLOBJ/LZ40_Decompress_SH4
 * (SH-4 ASM, MIT (c) 2024 VincentNL; original LZX/LZ40 format by CUE,
 * https://www.romhacking.net/utilities/826/, GPLv3) re-expressed here in
 * portable C so the same byte format decodes on host tools and on the SH-4
 * without extra tables, floats, mallocs or unaligned word loads. The vendored
 * SH-4 reference lives next to this file as LZ40_dec.asm (not built; this
 * header is what pack.c / dcfmv.c call).
 *
 ...
 *
 * Returns the decompressed size (== declen from the header) or < 0 on
 * malformed input. Stops at declen: callers may pass padding after the
 * stream (pack blocks are padded to 32 bytes, DCMV frames to 32).
 */
#include <stdint.h>
