...
Video *video_open_file(Ren *r, const char *path, float fps) { (void)r; (void)path; (void)fps; return NULL; }
bool   video_update(Video *v, float dt) { (void)v; (void)dt; return false; }
void   video_draw(Video *v, Ren *r, int sw, int sh) { (void)v; (void)r; (void)sw; (void)sh; }
void   video_draw_rect(Video *v, Ren *r, float x, float y, float w, float h) { (void)v; (void)r; (void)x; (void)y; (void)w; (void)h; }
